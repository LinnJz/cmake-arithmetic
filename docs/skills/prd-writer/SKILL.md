...
If the project repository already has a domain glossary, ADRs, or related PRDs, read them first and reuse their vocabulary and decisions — do not interview the user about facts already documented in the repo.
...
If a codebase exists or the user wants an engineering-ready PRD, also fill the optional sections in `references/prd-template.md` (五、实现决策 / 六、测试决策). Skip them for pure product-discovery PRDs.
...
If the user asks to publish the PRD to an issue tracker (e.g. GitHub), create the issue from the final PRD and apply the project's triage label if one exists.
